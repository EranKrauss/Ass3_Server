...
public class Tests implements Runnable {
    ...
    public String testRegisterCourseWithKdam(){
        String response = "";
        try {
            Course p= getCourseWithKdam();                          //course with kdam
            ...
            for (int i = 0; i < actualSpots+2; i++) {
                ...
                new Thread(() -> {
                    try {
                        ...
                        String s = "";
                        //ack 3
                        s = (commandsProcessors.get(finalI).sendCommand("LOGIN " + username + " " + username));
                        outputs.add(s);
                        //System.out.println(Thread.currentThread() + " :  " +s);
                        //error 9
                        s = (commandsProcessors.get(finalI).sendCommand("ISREGISTERED " + finalP.getCourseNum()));
                        outputs.add(s);
                        //System.out.println(Thread.currentThread() + " :  " +s);

                        //ack 5 + 2 error 5
                        s = (commandsProcessors.get(finalI).sendCommand("COURSEREG " + finalP.getKdamCourses().get(0).getCourseNum()));
                        outputs.add(s);
                        //System.out.println(Thread.currentThread() + " :  " +s);

                        //ack 5 + 2 error 5
                        s = (commandsProcessors.get(finalI).sendCommand("COURSEREG " + finalP.getCourseNum()));
                        outputs.add(s);
                        //System.out.println(Thread.currentThread() + " :  " +s);

                        //registered  , 2 error 9
                        s = (commandsProcessors.get(finalI).sendCommand("ISREGISTERED " + finalP.getCourseNum()));
                        outputs.add(s);
                        //System.out.println(Thread.currentThread() + " :  " +s);

                        //ack 10 , 2 error 10
                        s = (commandsProcessors.get(finalI).sendCommand("UNREGISTER " + finalP.getKdamCourses().get(0).getCourseNum()));
                        outputs.add(s);
                        //System.out.println(Thread.currentThread() + " :  " +s);

                        //ack 10 , 2 error 10
                        s = (commandsProcessors.get(finalI).sendCommand("UNREGISTER " + finalP.getCourseNum()));
                        outputs.add(s);
                        //System.out.println(Thread.currentThread() + " :  " +s);
    ...
    public void run() {
        try {
            ...
            new Thread(() -> {
                ReactorMain.main(new String[]{"7777"}); //You can change it to reactor also
